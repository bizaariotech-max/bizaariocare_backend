...
const ARRAY_LIMITS = {
  ChiefComplaints: 50,
  ClinicalFindings: 50,
  VitalsPhysicalExaminations: 100,
  DiagnosticsInvestigations: 100,
  Diagnosis: 50,
  PastMedications: 100,
  TreatmentMedicines: 50
};

// PRE-SAVE MIDDLEWARE - Enhanced for scalability
PatientProfilingSchema.pre("save", function (next) {
  ...
  // Implement array size limits to prevent document bloat
  this.limitArraySizes();
  
  next();
});

// Method to limit array sizes and archive old data
PatientProfilingSchema.methods.limitArraySizes = function() {
  // Limit ChiefComplaints - keep most recent
  if (this.ChiefComplaints && this.ChiefComplaints.length > ARRAY_LIMITS.ChiefComplaints) {
    const sorted = this.ChiefComplaints.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.ChiefComplaints = sorted.slice(0, ARRAY_LIMITS.ChiefComplaints);
  }
  
  // Limit ClinicalFindings - keep most recent
  if (this.ClinicalFindings && this.ClinicalFindings.length > ARRAY_LIMITS.ClinicalFindings) {
    const sorted = this.ClinicalFindings.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.ClinicalFindings = sorted.slice(0, ARRAY_LIMITS.ClinicalFindings);
  }
  
  // Limit VitalsPhysicalExaminations - keep most recent
  if (this.VitalsPhysicalExaminations && this.VitalsPhysicalExaminations.length > ARRAY_LIMITS.VitalsPhysicalExaminations) {
    const sorted = this.VitalsPhysicalExaminations.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.VitalsPhysicalExaminations = sorted.slice(0, ARRAY_LIMITS.VitalsPhysicalExaminations);
  }
  
  // Limit DiagnosticsInvestigations - keep most recent
  if (this.DiagnosticsInvestigations && this.DiagnosticsInvestigations.length > ARRAY_LIMITS.DiagnosticsInvestigations) {
    const sorted = this.DiagnosticsInvestigations.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.DiagnosticsInvestigations = sorted.slice(0, ARRAY_LIMITS.DiagnosticsInvestigations);
  }
  
  // Limit Diagnosis - keep most recent
  if (this.Diagnosis && this.Diagnosis.length > ARRAY_LIMITS.Diagnosis) {
    const sorted = this.Diagnosis.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.Diagnosis = sorted.slice(0, ARRAY_LIMITS.Diagnosis);
  }
  
  // Limit Past Medications - keep most recent
  if (this.MedicalSummary && this.MedicalSummary.PastMedications && 
      this.MedicalSummary.PastMedications.length > ARRAY_LIMITS.PastMedications) {
    const sorted = this.MedicalSummary.PastMedications.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.MedicalSummary.PastMedications = sorted.slice(0, ARRAY_LIMITS.PastMedications);
  }
  
  // Limit Treatment Medicines - keep most recent
  if (this.TreatmentToDate && this.TreatmentToDate.Medicines && 
      this.TreatmentToDate.Medicines.length > ARRAY_LIMITS.TreatmentMedicines) {
    const sorted = this.TreatmentToDate.Medicines.sort((a, b) => new Date(b.CreatedAt) - new Date(a.CreatedAt));
    this.TreatmentToDate.Medicines = sorted.slice(0, ARRAY_LIMITS.TreatmentMedicines);
  }
};
